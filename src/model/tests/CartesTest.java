...
public class CartesTest {
    ...
    @Test
    public void testB2Anglais_MarquerPlusieurJoueur() {
        B2Anglais b2 = new B2Anglais();
        ActionJoueur action = new ActionJoueur(alice, b2, null, null);
        ActionJoueur action2 = new ActionJoueur(bob, b2, null, null);

        b2.appliquerEffet(action, manche);
        b2.appliquerEffet(action2, manche);

        assertTrue(manche.getJoueursAyantJoueB2().contains(alice));
        assertTrue(manche.getJoueursAyantJoueB2().contains(bob));
    }
    ...
    // ==================== ANCIEN ÉLÈVE (Chancelier) ====================

    @Test
    public void testAncienEleve_PiocherEtGarder() {
        alice.reinitialiser();
        alice.recevoirCarte(new Exam()); // Carte en main initiale

        // S'assurer qu'il y a des cartes dans le deck
        Deck deck = manche.getDeck();
        int cartesAvant = deck.getNombreCartesRestantes();

        AncienEleve ancien = new AncienEleve();
        ActionJoueur action = new ActionJoueur(alice, ancien, null, null);
        action.setCarteGardeeIndex(0); // Garder la première carte (celle en main)

        ancien.appliquerEffet(action, manche);

        // Le joueur doit avoir 1 carte en main
        assertEquals(1, alice.getMain().getNombreCartes());
        // 2 cartes ont été piochées puis remises sous le deck
        // Le nombre de cartes dans le deck doit rester le même (pioche 2, remet 2)
    }

    @Test
    public void testAncienEleve_GarderCarteHauteValeur() {
        alice.reinitialiser();
        alice.recevoirCarte(new Exam()); // Valeur 1

        AncienEleve ancien = new AncienEleve();
        ActionJoueur action = new ActionJoueur(alice, ancien, null, null);
        action.setCarteGardeeIndex(-1); // Pas de choix explicite, devrait garder la plus haute

        ancien.appliquerEffet(action, manche);

        // Le joueur doit avoir gardé une carte
        assertEquals(1, alice.getMain().getNombreCartes());
    }

    @Test
    public void testAncienEleve_DeckInsuffisant() {
        alice.reinitialiser();
        alice.recevoirCarte(new Exam());
        ...
        // Le joueur garde sa carte originale
        assertEquals(1, alice.getMain().getNombreCartes());
    }

    @Test
    public void testAncienEleve_PasDeCible() {
        AncienEleve ancien = new AncienEleve();
        assertFalse(ancien.necessiteCible());
    }

    @Test
    public void testAncienEleve_NecessiteChoixCarte() {
        AncienEleve ancien = new AncienEleve();
        assertTrue(ancien.necessiteChoixCarte());
    }
